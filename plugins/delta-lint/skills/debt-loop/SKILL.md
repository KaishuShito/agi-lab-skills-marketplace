...
description: >
  Debt scoring and fix generation — prioritizes findings by info_score + roi_score,
  generates local fixes, shows diff. Does NOT commit, push, or create PRs.
...
| Does | Does NOT |
|------|----------|
| 優先度スコア計算（info_score + roi_score + severity_bonus） | git commit |
...
| `--dry-run` | **true (forced)** | Generate fixes but don't commit/push/PR |
